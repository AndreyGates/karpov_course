...
class SequentialForwardSelector:
    ...
    def __init__(
        ...
        # Bonferroni correction (done or not)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        # the checker var (will be used to track the best KFold scores)
        max_scores = np.full((self.cv.get_n_splits()), -1)
        ...
        for _ in rng_out:
            ...
            for feature in rng_in:
                ...
                scores = cross_val_score(self.model,
                                         ...
                                         scoring="r2",
                                         cv=self.cv, n_jobs=-1)
                # related one-tailed t-test
                ttest = stats.ttest_rel(max_scores, scores, alternative='less')
                _, p_value = ttest

                # significance level correction (based on the Bonferroni correction)
                corrected_alpha = self.alpha / len(excluded)\
                             if self.bonferroni else self.alpha

                # if the new feature significantly increased the cum. model perf, save it
                if p_value < corrected_alpha and max_scores.mean() < scores.mean():
                    argmax_feature = feature
                    max_scores = scores.copy()

            # if the optimization step gave out a feature
            # giving a better general perf
            if argmax_feature != -1 and global_score < max_scores.mean():
